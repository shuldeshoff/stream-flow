...
[![Coverage](https://img.shields.io/badge/coverage-35%25-yellow.svg)](https://github.com/shuldeshoff/stream-flow)
...
Banking API запускается на порту **8084** (SERVER_PORT + 4, по умолчанию 8080 + 4).

```bash
# Проверка транзакции
curl -X POST http://localhost:8084/api/banking/transactions \
  -H "Content-Type: application/json" \
  -d '{
...
curl http://localhost:8084/api/banking/cards/4532123456789012/limits

# Блокировка карты
curl -X POST http://localhost:8084/api/banking/cards/4532123456789012/block
```
...
Порты сервисов при дефолтном `SERVER_PORT=8080`:
...
| Query API | 8081 |
| WebSocket | 8082 |
| Auth API (если JWT включён) | 8083 |
| Banking API | 8084 |
| Prometheus metrics | 9090 |

Для CLI доступны переменные окружения:
- `STREAMFLOW_URL` - URL основного сервера (default: `http://localhost:8080`)
- `STREAMFLOW_QUERY_URL` - URL Query API (default: `http://localhost:8081`)
